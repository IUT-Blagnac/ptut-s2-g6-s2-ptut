 ...
 * L'ordre exécuté devait "traiter" une seule ligne et le résultat aboutit à 0 ou plusieurs lignes.
 * Exemple de use case de survenue : 
 * 		- demande d'un update d'un client à partir de son numéro 
 * 		- le numéro inexistant en base
 * 		- supérieure ou égale à 0 lignes updatées
 */
...
public class RowNotFoundOrTooManyRowsException extends ApplicationException {
	 ...
	 * @param cause		éventuelle autre Exception associée (en cas de SQLException principalement)
	 * @param rowsConcerned	nombre de lignes concernées (0 ou suppérieur à 1)
	 */
	public RowNotFoundOrTooManyRowsException(
			Table tablename, 
			Order order,
			String message,
			Throwable cause,
	...
	public String getMessage () {
		return super.getMessageAlone() 
				+ " (" + this.tablename + " - " + this.order 
				+ " - " + "(" + this.rowsConcerned + " rows)"
				+ " - " + this.getCause() + ")";
	}

...
public class LogToDatabase {
    ...
    // Global : accès à la base de donnée : à ajuster selon le compte Oracle disponible
    private static final String user = "pjts06"; // A modifier
    private static final String passwd = "ptut_g6_blagnac"; // A modifier
    ...
    /* Load du driver
     * Code dit "static" (hors d'une méthode) => exécuté au chargement de la classe
     * Permet de retrouver le diver => arrêt si driver non trouvé
     */
    {
        // driver oracle
    ...
    public static Connection getConnexion() throws DatabaseConnexionException {
        ...
        String url = "jdbc:oracle:thin:"
                + "@" + LogToDatabase.hoteOracle
                + ":" + LogToDatabase.portOracle
                + ":" + LogToDatabase.SIDOracle;

        try {
            LogToDatabase.currentConnexion =
                    DriverManager.getConnection(url, LogToDatabase.user, LogToDatabase.passwd);
